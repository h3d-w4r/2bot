...
async def main():
	...
	ketik(cyan+'	Username : '+kuning+f' {me.username}')
...
#doge
			...
			if hasattr(original_update.message,'reply_markup') and type(original_update.message.reply_markup) is ReplyInlineMarkup:
				...
				if url is not None:
					...
					if captcha is not None:
						ketik(kuning+'['+hijau+'BotDoge'+kuning+']'+cyan+f' Ada Captcha ==>>'+merah+' Skip CAPTCHA...');sleep(2)
	...
	async def no_ads(event):
		...
		if 'no new ads available' in message:	
			print(kuning+'['+hijau+'BotDoge'+kuning+']'+merah+' Maaf, Iklan Visit Habis '+reset)
			time.sleep(1)
			await client.send_message(doge, '/balance ')
	@client.on(events.NewMessage(chats=doge, incoming=True))
	async def balance(event):
		message = event.raw_text
		if 'Available balance:' in message:
			saldo = message.replace('Available balance: ','')
			saldos = saldo.replace(' DOGE','')
			print(kuning+'['+hijau+'BotDoge'+kuning+']'+cyan+f'{message}\n' +reset)
			time.sleep(1)
###### GANTI ALAMAT WALLET NYA , (jangan dihapus tanda petik dua [ " ] )
			wallet = "DTLawBtxZdqjiw2WrWfWajsKMLGkJDUr3o"

			if float(saldos) > 4:
				client.send_message(entity=doge,message="/withdraw")
				time.sleep(1)
				await client.send_message(entity=doge,message=wallet)
				time.sleep(1)
				await client.send_message(entity=doge,message=saldos)
				time.sleep(1)
				await client.send_message(entity=doge,message="/Confirm")
...
#ltc
	...
	async def visit_ads(event):
		...
		if type(original_update)is not UpdateShortMessage:
			if hasattr(original_update.message,'reply_markup') and type(original_update.message.reply_markup) is ReplyInlineMarkup:
				url = event.original_update.message.reply_markup.rows[0].buttons[0].url
				if url is not None:
					print(kuning+'['+abu+'BotLtc'+kuning+']'+cyan+f' Sedang Klik Link .....');sleep(2)
					(status_code, text_response) = get_response(url)
					parse_data = BeautifulSoup(text_response, 'html.parser')
					captcha = parse_data.find('div', {'class':'g-recaptcha'})
					tt=parse_data.find('div',{'id':'headbar'})
	...
	async def no_ads(event):
		message = event.raw_text
		if 'no new ads available' in message:	
			print(kuning+'['+abu+'BotLtc'+kuning+']'+merah+' Maaf, Iklan Visit Habis '+reset)
			sleep(1)
			await client.send_message(ltc, '/balance ')
	@client.on(events.NewMessage(chats=ltc, incoming=True))
	async def balance(event):
		message = event.raw_text
		if 'Available balance:' in message:
			saldo = message.replace('Available balance: ','')
			saldos = saldo.replace(' LTC','')
			print(kuning+'['+abu+'BotLtc'+kuning+']'+cyan+f'{message}\n' + reset)
			time.sleep(1)
## Ganti Wallet 
			wallet = "DTLawBtxZdqjiw2WrWfWajsKMLGkJDUr3o"
			if float(saldos) > 0.0004:
				client.send_message(entity=ltc,message="/withdraw")
				time.sleep(1)
				await client.send_message(entity=ltc,message=wallet)
				time.sleep(1)
				await client.send_message(entity=ltc,message=saldos)
				time.sleep(1)
				await client.send_message(entity=ltc,message="/Confirm")
...
#zec
	...
	async def visit_ads(event):
		...
		if type(original_update)is not UpdateShortMessage:
			if hasattr(original_update.message,'reply_markup') and type(original_update.message.reply_markup) is ReplyInlineMarkup:
				url = event.original_update.message.reply_markup.rows[0].buttons[0].url
				if url is not None:
					print(kuning+'['+pink+'BotZec'+kuning+']'+cyan+f' Sedang Klik Link .....');sleep(2)
					(status_code, text_response) = get_response(url)
					parse_data = BeautifulSoup(text_response, 'html.parser')
					captcha = parse_data.find('div', {'class':'g-recaptcha'})
					tt=parse_data.find('div',{'id':'headbar'})
	...
	async def balance(event):
		...
		if 'Available balance:' in message:
			...
			print(kuning+'['+pink+'BotZec'+kuning+']'+cyan+f'{message}\n' + Fore.RESET)
			time.sleep(1)
## Ganti Walletnya
			wallet = "DTLawBtxZdqjiw2WrWfWajsKMLGkJDUr3o"
			if float(saldos) > 0.00035:
				client.send_message(entity=zec,message="/withdraw")
				time.sleep(1)
				await client.send_message(entity=zec,message=wallet)
				time.sleep(1)
				await client.send_message(entity=zec,message=saldos)
				time.sleep(1)
				await client.send_message(entity=zec,message="/Confirm")
...
#bch
	...
	async def visit_ads(event):
		...
		if type(original_update)is not UpdateShortMessage:
			if hasattr(original_update.message,'reply_markup') and type(original_update.message.reply_markup) is ReplyInlineMarkup:
				url = event.original_update.message.reply_markup.rows[0].buttons[0].url
				if url is not None:
					ketik(kuning+f'['+reset+'BotBch'+kuning+']'+cyan+f' Sedang Klik Link .....');sleep(2)
					(status_code, text_response) = get_response(url)
					parse_data = BeautifulSoup(text_response, 'html.parser')
					captcha = parse_data.find('div', {'class':'g-recaptcha'})
					tt=parse_data.find('div',{'id':'headbar'})
	...
	async def no_ads(event):
		...
		if 'no new ads available' in message:	
			...
			time.sleep(1)
			await client.send_message(bch, '/balance ')
	@client.on(events.NewMessage(chats=bch, incoming=True))
	async def balance(event):
		message = event.raw_text
		if 'Available balance:' in message:
			saldo = message.replace('Available balance: ','')
			saldos = saldo.replace(' BCH','')
			print(kuning+'['+reset+'BotBch'+kuning+']'+cyan+ f'{message}\n' + reset)
			time.sleep(1)
#Ganti Wallet nya
			wallet = "DTLawBtxZdqjiw2WrWfWajsKMLGkJDUr3o"
			if float(saldos) > 0.0001:
				client.send_message(entity=bch,message="/withdraw")
				time.sleep(1)
				await client.send_message(entity=bch,message=wallet)
				time.sleep(1)
				await client.send_message(entity=bch,message=saldos)
				time.sleep(1)
				await client.send_message(entity=bch,message="/Confirm")
				sleep(1)
				ketik(merah+'		Nuyul BotTelegram Sudah Selsai '+reset)
